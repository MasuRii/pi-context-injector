// Vendored from agent/extensions/zellij-modal/modal.ts so this extension remains standalone.
// Keep this copy synchronized with the canonical modal primitives until zellij-modal is deprecated.
...
export class ZellijModalFrame {
	...
	private withBorderEdge(
		width: number,
		palette: ZellijColorPalette,
		position: "top" | "bottom",
		renderBody: (innerWidth: number, borderPaint: (text: string) => string) => string,
	): string {
		...
		if (innerWidth === 0) {
			const left = position === "top" ? this.borders.topLeft : this.borders.bottomLeft;
			const right = position === "top" ? this.borders.topRight : this.borders.bottomRight;
			return `${borderPaint(left)}${borderPaint(right)}`;
		}

		return renderBody(innerWidth, borderPaint);
	}

	private renderTitleBar(width: number, palette: ZellijColorPalette): string {
		return this.withBorderEdge(width, palette, "top", (innerWidth, borderPaint) => {
	...
	private renderBottomLine(width: number, palette: ZellijColorPalette): string {
		return this.withBorderEdge(width, palette, "bottom", (innerWidth, borderPaint) => {
...
export class ZellijModal implements ZellijModalComponent {
	...
	render(width: number): string[] {
		...
		try {
			...
			const padding = Array.from({ length: this.config.padding }, () => " ".repeat(paddedWidth));
	...
	private buildConfig(partial: ZellijModalConfigPartial): ZellijModalConfig {
		...
		const padding = finiteAtLeast(partial.padding, 1, 0);
		const minWidth = finiteAtLeast(partial.minWidth, 40, 4);
		const maxWidth = finiteAtLeast(partial.maxWidth, 0, 0);
		...
		return {
			...
			overlay: {
				...
				margin: finiteAtLeast(partial.overlay?.margin, 1, 0),
			},
		};
	}

	private resolveModalWidth(availableWidth: number): number {
		const width = finiteAtLeast(availableWidth, 4, 4);
...
function truncateWithGuard(text: string, maxWidth: number, body: () => string): string {
	if (visibleWidth(text) <= maxWidth) {
		return text;
	}
	if (maxWidth <= 1) {
...
function truncateStart(text: string, maxWidth: number): string {
	return truncateWithGuard(text, maxWidth, () => {
